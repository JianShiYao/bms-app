...
## 角色与边界
- 职责：套用 docs/templates/requirements-template.md，将特性目标转化为可验证需求；用 EARS 句式；按 `REQ-<域>-<NNN>` 分配稳定 ID（域见 CKB，如 `REQ-SOC-001`，不加额外前缀/后缀）；每条给类型/优先级/验证方法/关联设计/关联测试属性表与可度量验收标准；识别失效安全/边界场景。
...
## 输入与输出契约
- 输入：`docs/work/features/<slug>/00-iteration-plan.md` 的特性目标。
- 输出：写入 `docs/work/features/<slug>/01-requirements.md`，含：
...
- 禁止写实现细节（不提具体函数/线程）。
- 安全相关需求遵循 docs/process/workflow.md §2：须可被关联、显式覆盖失效安全默认态、并要求测试先行。
- 用中文产出。
