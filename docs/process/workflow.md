...
> 小 V 的**执行细节**(怎么调 agent 链、各阶段产出模板)见 [agents.md](agents.md) 与 [templates/](../templates);
> 本文只定义**流程骨架**与门禁触发时机。当前 CI 门禁事实表见
> [gates.md](../quality/gates.md)，避免在多处维护易漂移状态。

---

## 1. 特性开发生命周期(敏捷-V 小 V)

本节是**敏捷+V 方法论在本项目的操作落地**;方法论的"为什么/是什么"(模型、五条原则、可追溯性原理、DoR/DoD 的理由)见根基文档 [methodology.md](../concept/methodology.md),此处不再复述。下图为小 V 速查:
...
- **方法论依据**：[methodology.md](../concept/methodology.md)（敏捷+V 研发方法论根基；本节为其在本项目的操作落地）
- **agent 体系设计**：[superpowers/specs/2026-06-19-bms-agile-v-agents-design.md](../archive/specs/2026-06-19-bms-agile-v-agents-design.md)
- **怎么调 agent 链**：[agents.md](agents.md)(orchestrator → requirements → architect → designer → coder/tester → cicd)
- **端到端样例**：[features/soc-coulomb/](../work/features/soc-coulomb)

### 1.1 可追溯性(本项目落地)

> 为什么可追溯性是 V 模型的灵魂,见 [methodology.md §4 原则3](../concept/methodology.md)。本小节只给本项目的 ID 链与格式规则:
...
- **追溯矩阵独立成文**:每个特性维护 `docs/work/features/<slug>/traceability.md`(用
...
- 特性过程交付物落在 `docs/work/features/<slug>/`:`00-iteration-plan.md`(计划+派发清单)、`01-requirements.md`、
...
> DoR/DoD 的设立理由见 [methodology.md §5](../concept/methodology.md)。下为本项目落地清单:

每个特性默认继承以下通用判据(单特性可在其 `00-iteration-plan.md` 细化,但不得削弱)。

**准入(DoR,进入迭代前满足)**
- 依赖就绪(所需 `bms_db` entry / zbus 过渡通道 / 数据结构 / 配置项已存在或本迭代显式纳入);
...
- **变更已再基线**:本迭代若改动既有需求/安全项,其追溯链与受影响的右腿验证已同步更新(否则视为断链;依据 [methodology.md §4 原则6](../concept/methodology.md));
...
- `docs/work/features/<slug>/*` 过程交付物与 `app/`、`tests/` 代码**在同一个 PR** 提交——过程文档随实现一起评审、一起进 `master`。

## 2. 安全相关改动的额外要求

protection / 阈值 / 接触器 / 采样等**安全关键**改动,除常规流程外须额外满足(对齐项目失效安全红线:
默认接触器 **OPEN**,仅判定 NORMAL 才 CLOSED;另见 [management.md 第四节](../quality/management.md)):
...
> 方法论依据:[methodology.md §4 原则5](../concept/methodology.md)(持续合规)——**用隔离让冲突无从发生;实在要交汇,交给 git 在合并点显式裁决(可控集成)**。
...
- `check.ps1` 不是 Linux CI 的完整镜像；当前权威门禁状态见 [gates.md](../quality/gates.md)。
...
- **小 V 即一个 PR**：一个特性的全部小 V 交付物(`docs/work/features/<slug>/*`)+ `app/`/`tests/` 改动同 PR 提交;
...
| ⑤ CI / PR | push / PR | `.github/workflows/ci.yml`（见 [gates.md](../quality/gates.md)） | — | 分支保护拦合并 |
...
当前 CI 必过门禁以 [gates.md](../quality/gates.md) 为准。cppcheck/MISRA 已在 CI 中以
`CPPCHECK_FAIL=1` 阻断合并；本地 pre-push / `check.ps1` 仍保持 warn-only，用于快速反馈和
suppression 调整。各阶段质量管控现状与待补齐的全景见 [management.md](../quality/management.md)。
## 9. 分支保护说明

master 受保护，必过检查以 [gates.md](../quality/gates.md) 和 GitHub 分支保护配置为准。
