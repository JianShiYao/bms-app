...
> **定位**：本文细化 [architecture.md](architecture.md) 的 ADR-ARCH-005（诊断中心化），是诊断子系统的**权威设计契约**——规定 `bms_diag` 的**诊断条目（登记表）结构、severity 语义、去抖/锁存/老化生命周期、聚合到 `DB_DIAG_STATE`、以及故障到 `bms_bms` 状态机的路径**。**agent 据此实现或重构 `bms_diag` 代码，代码向本契约对齐**；本文不描述现状实现。
>
> **现状与差距**不在此维护：见 [architecture.md](architecture.md) §11 迁移路径（本契约在 **M3** 落地）。
>
> **规范措辞**：**必须 / 应 / 不得** 表示契约要求，可在评审、实现、测试中引用。相关：架构决策 [architecture.md](architecture.md) §8·§9，数据契约 [data-model.md](data-model.md)（`DB_DIAG_STATE`/`DB_PROT_STATE`），运行时 [runtime-model.md](runtime-model.md)（诊断在哪个任务消化），安全目标 [safety.md](safety.md)。
...
- **可追溯**：每个诊断条目有稳定 ID，可回指安全目标（[safety.md](safety.md) SG）与需求（追溯矩阵）。
...
- 时间比较**必须用有符号差值**以回绕安全（对齐 [runtime-model.md](runtime-model.md) §2）。
...
- **源失联 fail-safe**：若源模块本周期**未提供新鲜原始判定**（对应 `DB_*` entry stale 或缺失，见 [data-model.md](data-model.md) §stale），`bms_diag`**必须**：(a) **保持**该条目已 active 状态、**不得**自动 clear；(b) 置位一条"报告源失联/测量过期"诊断（其自身 severity 至少 ERROR）。
- 判 stale 复用统一时间基准 `bms_time_now_ms()`（[runtime-model.md](runtime-model.md) §2·§8）。

## 6. 聚合与 `DB_DIAG_STATE` 契约

`bms_diag` 每诊断周期把全部条目聚合为单一 entry，owner=`bms_diag`（[data-model.md](data-model.md)）。`DB_DIAG_STATE` **至少**表达：
...
| header | `timestamp_ms` / `sequence` / `validity`（统一 entry header，[data-model.md](data-model.md)） |

- 读者拿值拷贝，**不得**缓存 `bms_diag` 内部可变指针。
- 消费者：`bms_bms`（状态迁移，§7）、`bms_sys`（系统模式/授权）、`bms_comm`（上报）。
- `DB_DIAG_STATE` 的期望周期与 stale 容忍见 [data-model.md](data-model.md)（约 1–2 个诊断周期）。

## 7. 诊断 → `bms_bms` 迁移规则（契约）

`bms_bms` 把 `DB_DIAG_STATE` 作为状态机输入之一（ADR-ARCH-004：`bms_bms` 是接触器期望态 owner）。规则（**加严方向**）：

- `initialized == false` **或** `DB_DIAG_STATE` stale/invalid ⇒ 视为不安全 ⇒ **不得进入 NORMAL**（失效安全，[data-model.md](data-model.md)）。
...
下列模块**必须**将对应异常报告为诊断条目（原始判定），由 `bms_diag` 裁决（对齐 [architecture.md](architecture.md) §8）：
...
- `bms_sys_mon`：任务心跳超时、运行超时、栈余量不足、watchdog 门控异常（[runtime-model.md](runtime-model.md) §6·§7）。

## 10. 硬件 latch / zero-latency 集成（契约）

- 硬件严重故障（短路比较器、AFE ALERT）经 zero-latency ISR 只做最小安全动作（接触器强制 OPEN + 置硬件/软件 latch/event），**随后**由 `bms_diag` 将其登记为 CRITICAL 条目（[runtime-model.md](runtime-model.md) §3 zero-latency path、[architecture.md](architecture.md) §9）。
- 此类条目的 clear **必须**同时满足"硬件 latch 已解除"与 §8 授权；ISR 默认**不直接写** `bms_db`（[data-model.md](data-model.md)）。
...
- 至少一条 `源模块原始判定 → DIAG 聚合 → BMS 迁移` 的集成测试（对齐架构 §11 M3 与 [integration-test-strategy.md](../quality/integration-test-strategy.md)）。

## 12. 迁移

本契约落地阶段见 [architecture.md](architecture.md) §11：**M3 诊断中心化到 `bms_diag`**（完成判据：protection/meas/comm/sys_mon 故障进入诊断条目）。迁移期允许过渡实现与目标契约共存，但**目标以本契约为准**；每步须保持既有 CI 与 ztest 通过。

## 13. 参考

- [architecture.md](architecture.md) §8（ADR-ARCH-005）、§9、§11。
- [data-model.md](data-model.md)（`DB_DIAG_STATE` / `DB_PROT_STATE` / entry header / stale）。
- [runtime-model.md](runtime-model.md) §2（时间基准）、§6·§7（sys_mon / watchdog）、§8（stale）。
- foxBMS 2 Diagnosis / BMS Module（链接见 [architecture.md](architecture.md) §13）。
