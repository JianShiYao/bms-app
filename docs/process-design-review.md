...
> **文档定位**：本文补齐设计层的评审门槛，服务于 [process-workflow.md](process-workflow.md) 的小 V 流程与 [concept-methodology.md](concept-methodology.md) 的“变更即再基线 / 安全案例证据”原则。
...
- 设计说明：`DES-*` 或对应 `docs/features/<slug>/03-design.md`。
...
| 维度 | 问题 |
|------|------|
| 架构边界 | 是否符合 `concept-architecture.md` 的分层？是否引入反向依赖？ |
| 模块接口 | 是否符合 `standard-module-interface.md`？是否新增私有长期线程？ |
