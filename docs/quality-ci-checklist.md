...
| 能力 | bms-app 现状 | 状态 |
|---|---|---|
| 动态测试 + 覆盖率 | twister + ztest + gcovr（CI 门槛 行>=60%/分支>=38%） | ✅ 已落地 |
...
- [x] 核心步骤：`west twister`（测试+覆盖率，native_sim）→ 多板编译 → 静态分析（sca-gcc + clang-tidy + cppcheck/MISRA）→ 卫生门。
- [x] Job 结构：`format`（前置门）→ 并行 build / test / SCA / hygiene gates。
...
- [x] **cppcheck + MISRA**：`scripts/cppcheck-run.sh`，CI project mode 以 `CPPCHECK_FAIL=1` 阻断；pre-push / check.ps1 保持本地 warn-only 预检。噪声/deviation 在 `.cppcheck-suppressions` 维护，MISRA addon 不入库，本地或 CI 运行 `bash scripts/setup-cppcheck-misra.sh` 拉取。
- 说明：未采用 `codechecker` 是因为三条独立门能各自设阻断强度（CI 硬门、本地预检 warn-only），
  且免去 codechecker 额外依赖。

### P1 — 多板编译矩阵 ✅ 已落地
- [x] 用 build/twister 矩阵覆盖多板：CI `build` job 矩阵 = `mps2/an386` + `native_sim` + `qmxx_f407zg`；test-coverage 在 `native_sim` 跑。
- [x] 「每种配置都必须编过」的强制思想已体现（矩阵任一板失败即红）。
- [ ] `bms_f405` 暂从矩阵注释掉（板模板 dts/defconfig 未完善）；待板就绪后加入矩阵。

### P2 — 通用风格检查 ✅ 主要项已落地
- [x] `.editorconfig` 已就位（编辑器实时生效）。
- [x] CI 加 `editorconfig-checker`、`yamllint --strict`。
- [x] CI 加 `file-headers` 检查（SPDX + Doxygen 文件头）。
- [ ] Python 风格如需收紧，可后续加 `ruff` 或 `flake8`。
...
- [x] **SBOM（SPDX）**：用 Zephyr 原生 `west spdx`（无需新依赖）。
      流程：构建前 `west spdx --init -d build` → `west build` → `west spdx -d build`，
      产出 `build/spdx/*.spdx` 作为 release 资产上传。
- [x] **Release 产物签名**：在 `release.yml` 现有 SHA256SUMS 基础上，对 SHA256SUMS 签名
...
- [x] 已上免费 cppcheck misra addon；CI 已升为硬门，本地保持 warn-only 预检。
...
- [x] **cppcheck/MISRA 已作为 CI 硬门**：CI `cppcheck-misra` 使用 `CPPCHECK_FAIL=1`；本地 pre-push / check.ps1 仍 warn-only，避免 Windows 预检阻塞过重。
## 进度小结
1. ✅ **CI 骨架 + twister + 格式检查（P0）** —— 已完成。
2. ✅ **SCA（gcc）+ clang-tidy + cppcheck/MISRA（P1）** —— 已完成。
3. ⏳ **剩余可选项**：
   - P2：Python 风格工具、build provenance 可后续补。
   - P3：商业 MISRA 工具按合规需求评估。
